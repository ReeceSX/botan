...
class BOTAN_PUBLIC_API(2, 0) Buffered_Filter
{
  public:
    ...
    template <typename Alloc> void write(const std::vector<uint8_t, Alloc> &in, size_t length)
    {
 ...
 */
class BOTAN_PUBLIC_API(2, 0) Cipher_Mode_Filter final : public Keyed_Filter, private Buffered_Filter
{
  public:
 ...
 * @return pointer to the encryption or decryption filter
 */
inline Keyed_Filter *get_cipher(const std::string &algo_spec, const SymmetricKey &key, Cipher_Dir direction)
{
 ...
 */
inline Keyed_Filter *get_cipher(const std::string &algo_spec, const SymmetricKey &key, const InitializationVector &iv,
...
class BOTAN_PUBLIC_API(2, 0) Compression_Filter final : public Filter
{
  public:
    ...
    Compression_Filter(const std::string &type, size_t compression_level, size_t buffer_size = 4096);
