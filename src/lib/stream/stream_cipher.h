...
class BOTAN_PUBLIC_API(2, 0) StreamCipher : public SymmetricAlgorithm
{
  public:
     ...
     */
    static std::unique_ptr<StreamCipher> create_or_throw(const std::string &algo_spec,
     ...
     */
    template <typename Alloc> void encipher(std::vector<uint8_t, Alloc> &inout)
    {
     ...
     * @param inout the plaintext / ciphertext
     */
    template <typename Alloc> void encrypt(std::vector<uint8_t, Alloc> &inout)
    {
        cipher(inout.data(), inout.data(), inout.size());
    }
     ...
     * @param inout the plaintext / ciphertext
     */
    template <typename Alloc> void decrypt(std::vector<uint8_t, Alloc> &inout)
    {
        cipher(inout.data(), inout.data(), inout.size());
    }
