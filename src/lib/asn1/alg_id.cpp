 ...
 * Create an AlgorithmIdentifier
 */
AlgorithmIdentifier::AlgorithmIdentifier(const std::string &oid, const std::vector<uint8_t> &param)
 ...
 * Create an AlgorithmIdentifier
 */
AlgorithmIdentifier::AlgorithmIdentifier(const std::string &oid, Encoding_Option option)
...
{
    codec.start_cons(SEQUENCE).encode(oid()).raw_bytes(parameters()).end_cons();
}
...
{
    codec.start_cons(SEQUENCE).decode(m_oid).raw_bytes(m_parameters).end_cons();
}
