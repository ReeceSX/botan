...
{
    BOTAN_ARG_CHECK(m_tag == UTC_TIME || m_tag == GENERALIZED_TIME, "ASN1_Time: Bad encoding tag");
...
std::string ASN1_Time::to_string() const
{
    ...
    if (m_tag == UTC_TIME)
    {
        if (m_year < 1950 || m_year >= 2050)
            throw Encoding_Error("ASN1_Time: The time " + readable_string() + " cannot be encoded as a UTCTime");
