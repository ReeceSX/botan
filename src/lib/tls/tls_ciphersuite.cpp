...
{
    return auth_method() != Auth_Method::ANONYMOUS && auth_method() != Auth_Method::IMPLICIT;
}
...
{
    return (BlockCipher::providers(cipher).size() > 0) || (StreamCipher::providers(cipher).size() > 0);
}
