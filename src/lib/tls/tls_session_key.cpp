 ...
 */
Session_Keys::Session_Keys(const Handshake_State *state, const secure_vector<uint8_t> &pre_master_secret, bool resuming)
{
    ...
    const uint8_t MASTER_SECRET_MAGIC[] = {0x6D, 0x61, 0x73, 0x74, 0x65, 0x72, 0x20,
    ...
    else
    {
        ...
        if (extended_master_secret)
        {
            ...
            salt += state->hash().final(state->version(), state->ciphersuite().prf_algo());
        }
        else
        {
    ...
    const secure_vector<uint8_t> prf_output = prf->derive_key(prf_gen, m_master_sec.data(), m_master_sec.size(),
