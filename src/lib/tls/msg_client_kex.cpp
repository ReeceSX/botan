 ...
 */
Client_Key_Exchange::Client_Key_Exchange(Handshake_IO &io, Handshake_State &state, const Policy &policy,
                                         Credentials_Manager &creds, const Public_Key *server_public_key,
                                         const std::string &hostname, RandomNumberGenerator &rng)
{
    ...
    if (kex_algo == Kex_Algo::PSK)
    {
        ...
        const std::string psk_identity = creds.psk_identity("tls-client", hostname, identity_hint);
    ...
    else if (state.server_kex())
    {
        ...
        else if (kex_algo == Kex_Algo::ECDH || kex_algo == Kex_Algo::ECDHE_PSK)
        {
            ...
            {
                throw TLS_Exception(Alert::HANDSHAKE_FAILURE, "Server sent ECC curve prohibited by policy");
            }
        ...
        else if (kex_algo == Kex_Algo::SRP_SHA)
        {
            ...
            const std::string srp_password = creds.srp_password("tls-client", hostname, srp_identifier);

            std::pair<BigInt, SymmetricKey> srp_vals =
                srp6_client_agree(srp_identifier, srp_password, srp_group, "SHA-1", salt, B, rng);
 ...
 */
Client_Key_Exchange::Client_Key_Exchange(const std::vector<uint8_t> &contents, const Handshake_State &state,
                                         const Private_Key *server_rsa_kex_key, Credentials_Manager &creds,
                                         const Policy &policy, RandomNumberGenerator &rng)
{
    const Kex_Algo kex_algo = state.ciphersuite().kex_method();
    ...
    if (kex_algo == Kex_Algo::STATIC_RSA)
    {
        ...
        m_pre_master = decryptor.decrypt_or_random(encrypted_pre_master.data(), encrypted_pre_master.size(),
                                                   expected_plaintext_size, rng, expected_content_bytes,
                                                   expected_content_pos, expected_content_size);
    }
    else
    {
        TLS_Data_Reader reader("ClientKeyExchange", contents);
        ...
        if (key_exchange_is_psk(kex_algo))
        {
            ...
            if (psk.length() == 0)
            {
                if (policy.hide_unknown_users())
                    ...
                    throw TLS_Exception(Alert::UNKNOWN_PSK_IDENTITY, "No PSK for identifier " + psk_identity);
            }
        }
...
#endif
        else if (kex_algo == Kex_Algo::DH || kex_algo == Kex_Algo::DHE_PSK || kex_algo == Kex_Algo::ECDH ||
                 kex_algo == Kex_Algo::ECDHE_PSK)
        {
            const Private_Key &private_key = state.server_kex()->server_kex_key();

            const PK_Key_Agreement_Key *ka_key = dynamic_cast<const PK_Key_Agreement_Key *>(&private_key);

            if (!ka_key)
                throw Internal_Error("Expected key agreement key type but got " + private_key.algo_name());
