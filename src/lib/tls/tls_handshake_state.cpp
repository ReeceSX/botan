...
const char *handshake_type_to_string(Handshake_Type type)
{
    ...
    throw TLS_Exception(Alert::UNEXPECTED_MESSAGE, "Unknown TLS handshake message type " + std::to_string(type));
}
...
std::string handshake_mask_to_string(uint32_t mask, char combiner)
{
    const Handshake_Type types[] = {
        HELLO_VERIFY_REQUEST, HELLO_REQUEST,      CLIENT_HELLO,       SERVER_HELLO,        CERTIFICATE,
        CERTIFICATE_URL,      CERTIFICATE_STATUS, SERVER_KEX,         CERTIFICATE_REQUEST, SERVER_HELLO_DONE,
        CERTIFICATE_VERIFY,   CLIENT_KEX,         NEW_SESSION_TICKET, HANDSHAKE_CCS,       FINISHED};
...
Handshake_State::Handshake_State(Handshake_IO *io, Callbacks &cb)
    : m_callbacks(cb), m_handshake_io(io), m_version(m_handshake_io->initial_record_version())
{
}
...
{
    const bool expecting_ccs = (bitmask_for_handshake_type(HANDSHAKE_CCS) & m_hand_expecting_mask) != 0;
...
bool supported_algos_include(const std::vector<Signature_Scheme> &schemes, const std::string &key_type,
                             const std::string &hash_type)
{
    for (Signature_Scheme scheme : schemes)
    {
        if (signature_scheme_is_known(scheme) && hash_function_of_scheme(scheme) == hash_type &&
...
std::pair<std::string, Signature_Format> Handshake_State::parse_sig_format(const Public_Key &key,
                                                                           ...
                                                                           const Policy &policy) const
{
    ...
    {
        throw TLS_Exception(Alert::HANDSHAKE_FAILURE, "Rejecting " + key_type + " signature");
    }
    ...
    {
        throw TLS_Exception(Alert::HANDSHAKE_FAILURE, "No certificate verify set");
    }
