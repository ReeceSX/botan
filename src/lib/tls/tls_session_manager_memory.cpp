...
Session_Manager_In_Memory::Session_Manager_In_Memory(RandomNumberGenerator &rng, size_t max_sessions,
                                                     std::chrono::seconds session_lifetime)
    : m_max_sessions(max_sessions), m_session_lifetime(session_lifetime), m_rng(rng),
      m_session_key(m_rng.random_vec(32))
{
}

bool Session_Manager_In_Memory::load_from_session_str(const std::string &session_str, Session &session)
{
    // assert(lock is held)
...
bool Session_Manager_In_Memory::load_from_session_id(const std::vector<uint8_t> &session_id, Session &session)
{
...
bool Session_Manager_In_Memory::load_from_server_info(const Server_Information &info, Session &session)
{
    lock_guard_type<mutex_type> lock(m_mutex);
...
void Session_Manager_In_Memory::remove_entry(const std::vector<uint8_t> &session_id)
{
    lock_guard_type<mutex_type> lock(m_mutex);
