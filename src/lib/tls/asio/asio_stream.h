 ...
 */
template <class StreamLayer, class ChannelT = Channel> class Stream
{
  public:
    //! \name construction
    //! @{
    ...
    explicit Stream(Context &context, Args &&... args)
        : m_context(context), m_nextLayer(std::forward<Args>(args)...), m_core(*this), m_shutdown_received(false),
    ...
    explicit Stream(Arg &&arg, Context &context)
        : m_context(context), m_nextLayer(std::forward<Arg>(arg)), m_core(*this), m_shutdown_received(false),
          m_input_buffer_space(MAX_CIPHERTEXT_SIZE, '\0'),
          m_input_buffer(m_input_buffer_space.data(), m_input_buffer_space.size())
    {
    }
    ...
    //! @throws Not_Implemented
    template <typename verify_mode> void set_verify_mode(verify_mode v)
    {
        BOTAN_UNUSED(v);
     ...
     * @param ec Will be set to `Botan::ErrorType::NotImplemented`
     */
    template <typename verify_mode> void set_verify_mode(verify_mode v, boost::system::error_code &ec)
    {
        BOTAN_UNUSED(v);
        ec = Botan::ErrorType::NotImplemented;
    }
        ...
        -> BOOST_ASIO_INITFN_RESULT_TYPE(HandshakeHandler, void(boost::system::error_code))
    {
        ...
        detail::AsyncHandshakeOperation<typename std::decay<HandshakeHandler>::type, Stream> op{
            std::move(init.completion_handler), *this, ec};
  ...
  public:
    ...
    template <typename ShutdownHandler> void async_shutdown(ShutdownHandler &&handler)
    {
        ...
        boost::asio::async_completion<ShutdownHandlerWrapper, void(boost::system::error_code, std::size_t)> init(w);

        detail::AsyncWriteOperation<typename std::decay<ShutdownHandlerWrapper>::type, Stream> op{
            std::move(init.completion_handler), *this, boost::asio::buffer_size(send_buffer())};
        ...
        -> BOOST_ASIO_INITFN_RESULT_TYPE(WriteHandler, void(boost::system::error_code, std::size_t))
    {
        ...
        if (ec)
        {
            ...
            detail::AsyncWriteOperation<typename std::decay<WriteHandler>::type, Stream> op{
                std::move(init.completion_handler), *this, std::size_t(0), ec};
            return init.result.get();
        }

        detail::AsyncWriteOperation<typename std::decay<WriteHandler>::type, Stream> op{
            std::move(init.completion_handler), *this, boost::asio::buffer_size(buffers)};
        ...
        -> BOOST_ASIO_INITFN_RESULT_TYPE(ReadHandler, void(boost::system::error_code, std::size_t))
    {
        ...
        detail::AsyncReadOperation<typename std::decay<ReadHandler>::type, Stream, MutableBufferSequence> op{
  ...
  protected:
    ...
    class StreamCore : public Botan::TLS::Callbacks
    {
      public:
        ...
        {
            m_stream.m_receive_buffer.commit(boost::asio::buffer_copy(m_stream.m_receive_buffer.prepare(size),
        ...
        void tls_verify_cert_chain(const std::vector<X509_Certificate> &cert_chain,
                                   ...
                                   const std::vector<Certificate_Store *> &trusted_roots, Usage_Type usage,
                                   const std::string &hostname, const TLS::Policy &policy) override
        {
            if (m_stream.m_context.has_verify_callback())
            {
                m_stream.m_context.get_verify_callback()(cert_chain, ocsp_responses, trusted_roots, usage, hostname,
                                                         policy);
            }
            else
            {
    ...
    template <typename MutableBufferSequence> std::size_t copy_received_data(MutableBufferSequence buffers)
    {
    ...
    typename std::enable_if<!std::is_same<Channel, T>::value>::type setup_native_handle(Connection_Side,
     ...
     */
    template <class T = ChannelT>
    typename std::enable_if<std::is_same<Channel, T>::value>::type setup_native_handle(Connection_Side side,
                                                                                       boost::system::error_code &ec)
    {
        try_with_error_code(
            [&] {
                if (side == CLIENT)
                {
                    m_native_handle = std::unique_ptr<Client>(new Client(
                        m_core, m_context.m_session_manager, m_context.m_credentials_manager, m_context.m_policy,
                        m_context.m_rng, m_context.m_server_info, Protocol_Version::latest_tls_version()));
                }
                else
                {
                    m_native_handle = std::unique_ptr<Server>(
                        new Server(m_core, m_context.m_session_manager, m_context.m_credentials_manager,
                                   m_context.m_policy, m_context.m_rng, false /* no DTLS */));
                }
            },
            ec);
    }
    ...
    void tls_encrypt(const ConstBufferSequence &buffers, boost::system::error_code &ec)
    {
        ...
        for (auto it = boost::asio::buffer_sequence_begin(buffers);
             !ec && it != boost::asio::buffer_sequence_end(buffers); it++)
        {
            const boost::asio::const_buffer buffer = *it;
            try_with_error_code(
                [&] { native_handle()->send(static_cast<const uint8_t *>(buffer.data()), buffer.size()); }, ec);
        }
    }
    ...
    template <typename Fun> void try_with_error_code(Fun f, boost::system::error_code &ec)
    {
        try
        {
            f();
