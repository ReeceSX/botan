...
{
  public:
    Client_Handshake_State(Handshake_IO *io, Callbacks &cb) : Handshake_State(io, cb), m_is_reneg(false)
    {
    }
    ...
    {
        return (resumed_session != nullptr);
    }
 ...
 */
Client::Client(Callbacks &callbacks, Session_Manager &session_manager, Credentials_Manager &creds, const Policy &policy,
               RandomNumberGenerator &rng, const Server_Information &info, const Protocol_Version &offer_version,
               const std::vector<std::string> &next_protocols, size_t io_buf_sz)
    : Channel(callbacks, session_manager, rng, policy, false, offer_version.is_datagram_protocol(), io_buf_sz),
      m_creds(creds), m_info(info)
{
    ...
    send_client_hello(state, false, offer_version, srp_identifier, next_protocols);
}
...
std::vector<X509_Certificate> Client::get_peer_cert_chain(const Handshake_State &state) const
{
 ...
 */
void Client::initiate_handshake(Handshake_State &state, bool force_full_renegotiation)
{
    send_client_hello(state, force_full_renegotiation,
                      policy().latest_supported_version(state.version().is_datagram_protocol()));
}

void Client::send_client_hello(Handshake_State &state_base, bool force_full_renegotiation, Protocol_Version version,
                               const std::string &srp_identifier, const std::vector<std::string> &next_protocols)
{
        ...
        if (session_manager().load_from_server_info(m_info, *session_info))
        {
            ...
            */
            ...
            if (policy().acceptable_ciphersuite(session_info->ciphersuite()) && session_version_ok)
            {
                ...
                {
                    state.client_hello(new Client_Hello(state.handshake_io(), state.hash(), policy(), callbacks(),
                                                        rng(), secure_renegotiation_data_for_client_hello(),
                                                        *session_info, next_protocols));
    ...
    if (!state.client_hello()) // not resuming
    {
        ...
        state.client_hello(new Client_Hello(state.handshake_io(), state.hash(), policy(), callbacks(), rng(),
                                            secure_renegotiation_data_for_client_hello(), client_settings,
                                            next_protocols));
    }
 ...
 */
void Client::process_handshake_msg(const Handshake_State *active_state, Handshake_State &state_base,
                                   Handshake_Type type, const std::vector<uint8_t> &contents, bool epoch0_restart)
{
    ...
    else if (type == SERVER_HELLO)
    {
        ...
        {
            throw TLS_Exception(Alert::HANDSHAKE_FAILURE, "Server replied with ciphersuite we didn't send");
        }
        ...
        {
            throw TLS_Exception(Alert::HANDSHAKE_FAILURE, "Server replied with a signaling ciphersuite");
        }

        if (state.server_hello()->compression_method() != 0)
        {
            throw TLS_Exception(Alert::ILLEGAL_PARAMETER, "Server replied with non-null compression method");
        }
        ...
        std::set_difference(server_extn.begin(), server_extn.end(), client_extn.begin(), client_extn.end(),
        ...
        if (uint16_t srtp = state.server_hello()->srtp_profile())
        {
            if (!value_exists(state.client_hello()->srtp_profiles(), srtp))
                throw TLS_Exception(Alert::HANDSHAKE_FAILURE, "Server replied with DTLS-SRTP alg we did not send");
        }
        ...
        if (server_returned_same_session_id)
        {
            ...
            if (state.server_hello()->version() != state.client_hello()->version())
                throw TLS_Exception(Alert::HANDSHAKE_FAILURE, "Server resumed session but with wrong version");
        ...
        else
        {
            ...
            if (active_state)
            {
                ...
                if (active_state->version() != state.server_hello()->version())
                    throw TLS_Exception(Alert::PROTOCOL_VERSION, "Server changed version after renegotiation");
            ...
            {
                throw TLS_Exception(Alert::HANDSHAKE_FAILURE, "Server replied with later version than client offered");
            }
    ...
    else if (type == CERTIFICATE)
    {
        ...
        if (server_certs.empty())
            throw TLS_Exception(Alert::HANDSHAKE_FAILURE, "Client: No certificates sent by server");
        ...
        */
        ...
        if (peer_key->algo_name() != expected_key_type)
            throw TLS_Exception(Alert::ILLEGAL_PARAMETER, "Certificate key type did not match ciphersuite");

        if (!key_usage_matches_ciphersuite(server_cert.constraints(), state.ciphersuite()))
            throw TLS_Exception(Alert::BAD_CERTIFICATE, "Certificate usage constraints do not allow this ciphersuite");
        ...
        else
        {
            try
            {
                ...
                callbacks().tls_verify_cert_chain(server_certs, {}, trusted_CAs, Usage_Type::TLS_SERVER_AUTH,
                                                  m_info.hostname(), policy());
            }
            catch (TLS_Exception &)
            {
                throw;
    ...
    else if (type == SERVER_KEX)
    {
        ...
        state.server_kex(new Server_Key_Exchange(contents, state.ciphersuite().kex_method(),
                                                 state.ciphersuite().auth_method(), state.version()));
        ...
        if (state.ciphersuite().signature_used())
        {
            ...
            {
                throw TLS_Exception(Alert::DECRYPT_ERROR, "Bad signature on server key exchange");
            }
        }
    }
    ...
    else if (type == SERVER_HELLO_DONE)
    {
        ...
        if (state.server_certs() != nullptr && state.server_hello()->supports_certificate_status_message())
        {
            try
            {
                ...
                callbacks().tls_verify_cert_chain(state.server_certs()->cert_chain(), ocsp, trusted_CAs,
                                                  Usage_Type::TLS_SERVER_AUTH, m_info.hostname(), policy());
            }
            catch (TLS_Exception &)
            {
                throw;
        ...
        state.client_kex(new Client_Key_Exchange(state.handshake_io(), state, policy(), m_creds,
                                                 state.server_public_key.get(), m_info.hostname(), rng()));

        state.compute_session_keys();

        if (state.received_handshake_msg(CERTIFICATE_REQUEST) && !state.client_certs()->empty())
        {
            Private_Key *private_key =
                m_creds.private_key_for(state.client_certs()->cert_chain()[0], "tls-client", m_info.hostname());

            state.client_verify(new Certificate_Verify(state.handshake_io(), state, policy(), rng(), private_key));
        }
    ...
    else if (type == FINISHED)
    {
        ...
        Session session_info(session_id, state.session_keys().master_secret(), state.server_hello()->version(),
                             state.server_hello()->ciphersuite(), CLIENT,
                             state.server_hello()->supports_extended_master_secret(),
                             state.server_hello()->supports_encrypt_then_mac(), get_peer_cert_chain(state),
                             session_ticket, m_info, "", state.server_hello()->srtp_profile());
