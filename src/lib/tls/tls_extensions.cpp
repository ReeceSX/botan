...
Extension *make_extension(TLS_Data_Reader &reader, uint16_t code, uint16_t size, Connection_Side from)
{
    ...
    return new Unknown_Extension(static_cast<Handshake_Extension_Type>(code), reader, size);
}
    ...
    if (reader.has_remaining())
    {
        ...
        while (reader.has_remaining())
        {
            ...
            if (m_extensions.find(type) != m_extensions.end())
                throw TLS_Exception(TLS::Alert::DECODE_ERROR, "Peer sent duplicated extensions");

            Extension *extn = make_extension(reader, extension_code, extension_size, from);
...
Unknown_Extension::Unknown_Extension(Handshake_Extension_Type type, TLS_Data_Reader &reader, uint16_t extension_size)
...
SRP_Identifier::SRP_Identifier(TLS_Data_Reader &reader, uint16_t extension_size)
...
const std::string &Application_Layer_Protocol_Notification::single_protocol() const
{
    if (m_protocols.size() != 1)
        throw TLS_Exception(Alert::HANDSHAKE_FAILURE, "Server sent " + std::to_string(m_protocols.size()) +
...
Session_Ticket::Session_Ticket(TLS_Data_Reader &reader, uint16_t extension_size)
...
Extended_Master_Secret::Extended_Master_Secret(TLS_Data_Reader &, uint16_t extension_size)
{
    if (extension_size != 0)
...
Encrypt_then_MAC::Encrypt_then_MAC(TLS_Data_Reader &, uint16_t extension_size)
{
    if (extension_size != 0)
...
Certificate_Status_Request::Certificate_Status_Request(TLS_Data_Reader &reader, uint16_t extension_size,
...
Certificate_Status_Request::Certificate_Status_Request(const std::vector<uint8_t> &ocsp_responder_ids,
    ...
    : m_ocsp_names(ocsp_responder_ids), m_ocsp_keys(ocsp_key_ids)
{
}
