...
class Connection_Cipher_State final
{
  public:
     ...
     */
    Connection_Cipher_State(Protocol_Version version, Connection_Side which_side, bool is_our_side,
                            const Ciphersuite &suite, const Session_Keys &keys, bool uses_encrypt_then_mac);
...
class Record_Header final
{
  public:
    ...
    Record_Header(size_t needed) : m_needed(needed), m_sequence(0), m_version(Protocol_Version()), m_type(NO_RECORD)
    {
    }
 ...
 */
void write_unencrypted_record(secure_vector<uint8_t> &write_buffer, uint8_t record_type,
                              Protocol_Version record_version, uint64_t record_sequence, const uint8_t *message,
                              size_t message_len);
 ...
 */
void write_record(secure_vector<uint8_t> &write_buffer, uint8_t record_type, Protocol_Version record_version,
                  uint64_t record_sequence, const uint8_t *message, size_t message_len,
                  Connection_Cipher_State &cipherstate, RandomNumberGenerator &rng);
 ...
 */
Record_Header read_record(bool is_datagram, secure_vector<uint8_t> &read_buffer, const uint8_t input[],
                          size_t input_len, size_t &consumed, secure_vector<uint8_t> &record_buf,
