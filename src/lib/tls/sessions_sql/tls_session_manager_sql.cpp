...
Session_Manager_SQL::Session_Manager_SQL(std::shared_ptr<SQL_Database> db, const std::string &passphrase,
                                         RandomNumberGenerator &rng, size_t max_sessions,
                                         std::chrono::seconds session_lifetime)
    : m_db(db), m_rng(rng), m_max_sessions(max_sessions), m_session_lifetime(session_lifetime)
{
    m_db->create_table("create table if not exists tls_sessions "
                       "("
    ...
    m_db->create_table("create table if not exists tls_sessions_metadata "
                       "("
    ...
    if (salts == 1)
    {
        ...
        if (stmt->step())
        {
            ...
            secure_vector<uint8_t> x = pbkdf->pbkdf_iterations(32 + 2, passphrase, salt.first, salt.second, iterations);
    ...
    else
    {
        ...
        secure_vector<uint8_t> x = pbkdf->pbkdf_timed(32 + 2, passphrase, salt.data(), salt.size(),
                                                      std::chrono::milliseconds(100), iterations);
...
bool Session_Manager_SQL::load_from_session_id(const std::vector<uint8_t> &session_id, Session &session)
{
...
bool Session_Manager_SQL::load_from_server_info(const Server_Information &server, Session &session)
{
