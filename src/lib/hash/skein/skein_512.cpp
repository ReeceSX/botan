...
Skein_512::Skein_512(size_t arg_output_bits, const std::string &arg_personalization)
    : m_personalization(arg_personalization), m_output_bits(arg_output_bits), m_threefish(new Threefish_512), m_T(2),
      m_buffer(64), m_buf_pos(0)
{
...
std::string Skein_512::name() const
{
    if (m_personalization != "")
        return "Skein-512(" + std::to_string(m_output_bits) + "," + m_personalization + ")";
...
void Skein_512::reset_tweak(type_code type, bool is_final)
{
    ...
    m_T[1] = (static_cast<uint64_t>(type) << 56) | (static_cast<uint64_t>(1) << 62) |
