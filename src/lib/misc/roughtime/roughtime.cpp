...
template <typename T> T impl_from_little_endian(const uint8_t *t, const size_t i)
{
    static_assert(sizeof(T) <= sizeof(int64_t), "");
    return T(static_cast<int64_t>(t[i]) << i * 8) + (i == 0 ? T(0) : impl_from_little_endian<T>(t, i - 1));
}

template <typename T> T from_little_endian(const uint8_t *t)
{
    return impl_from_little_endian<T>(t, sizeof(T) - 1);
}

template <typename T, enable_if_t<is_array<T>::value> * = nullptr> T copy(const uint8_t *t)
{
...
template <typename T> std::map<std::string, std::vector<uint8_t>> unpack_roughtime_packet(T bytes)
{
    if (bytes.size() < 8)
    {
...
template <typename T> T get(const std::map<std::string, std::vector<uint8_t>> &map, const std::string &label)
{
...
template <size_t N, typename T> std::array<uint8_t, N> vector_to_array(std::vector<uint8_t, T> vec)
{
    if (vec.size() != N)
    {
...
void Chain::append(const Link &new_link, size_t max_chain_size)
{
    ...
    while (m_links.size() >= max_chain_size)
    {
        ...
        if (m_links.size() >= 2)
        {
            m_links[1].nonce_or_blind() = nonce_from_blind(
                m_links[0].response(), m_links[1].nonce_or_blind()); // we need to convert blind to nonce
        }
