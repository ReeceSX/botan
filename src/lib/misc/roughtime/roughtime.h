...
class BOTAN_PUBLIC_API(2, 13) Response final
{
  ...
  private:
    Response(const std::array<uint8_t, 72> &dele, const std::array<uint8_t, 64> &sig, sys_microseconds64 utc_midp,
 ...
 */
BOTAN_PUBLIC_API(2, 13)
std::vector<uint8_t> online_request(const std::string &url, const Nonce &nonce,
                                    std::chrono::milliseconds timeout = std::chrono::seconds(3));

struct BOTAN_PUBLIC_API(2, 13) Server_Information final
{
  public:
    Server_Information(const std::string &name, const Botan::Ed25519_PublicKey &public_key,
