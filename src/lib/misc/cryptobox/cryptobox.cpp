...
std::string encrypt(const uint8_t input[], size_t input_len, const std::string &passphrase, RandomNumberGenerator &rng)
{
    ...
    */
    ...
    OctetString master_key = pbkdf->derive_key(CIPHER_KEY_LEN + MAC_KEY_LEN + CIPHER_IV_LEN, passphrase,
                                               &out_buf[VERSION_CODE_LEN], PBKDF_SALT_LEN, PBKDF_ITERATIONS);
...
secure_vector<uint8_t> decrypt_bin(const uint8_t input[], size_t input_len, const std::string &passphrase)
{
    ...
    OctetString master_key =
        pbkdf->derive_key(PBKDF_OUTPUT_LEN, passphrase, pbkdf_salt, PBKDF_SALT_LEN, PBKDF_ITERATIONS);
