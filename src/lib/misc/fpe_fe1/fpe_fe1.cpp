...
FPE_FE1::FPE_FE1(const BigInt &n, size_t rounds, bool compat_mode, const std::string &mac_algo) : m_rounds(rounds)
{
    if (m_rounds < 3)
...
BigInt FPE_FE1::F(const BigInt &R, size_t round, const secure_vector<uint8_t> &tweak_mac,
