 ...
 */
std::pair<BigInt, SymmetricKey> BOTAN_PUBLIC_API(2, 0)
    srp6_client_agree(const std::string &username, const std::string &password, const std::string &group_id,
                      const std::string &hash_id, const std::vector<uint8_t> &salt, const BigInt &B,
...
std::pair<BigInt, SymmetricKey> BOTAN_PUBLIC_API(2, 11)
    srp6_client_agree(const std::string &username, const std::string &password, const DL_Group &group,
                      const std::string &hash_id, const std::vector<uint8_t> &salt, const BigInt &B, size_t a_bits,
                      RandomNumberGenerator &rng);
...
BigInt BOTAN_PUBLIC_API(2, 0)
    generate_srp6_verifier(const std::string &identifier, const std::string &password, const std::vector<uint8_t> &salt,
...
class BOTAN_PUBLIC_API(2, 0) SRP6_Server_Session final
{
  public:
     ...
     * @return SRP-6 B value
     */
    BigInt step1(const BigInt &v, const DL_Group &group, const std::string &hash_id, const size_t b_bits,
                 RandomNumberGenerator &rng);
