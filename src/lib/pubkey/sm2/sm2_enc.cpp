...
{
  public:
    SM2_Encryption_Operation(const SM2_Encryption_PublicKey &key, RandomNumberGenerator &rng,
                             const std::string &kdf_hash)
        : m_group(key.domain()), m_kdf_hash(kdf_hash), m_ws(PointGFp::WORKSPACE_SIZE),
    ...
    secure_vector<uint8_t> encrypt(const uint8_t msg[], size_t msg_len, RandomNumberGenerator &rng) override
    {
        ...
        const secure_vector<uint8_t> kdf_output = kdf->derive_key(msg_len, kdf_input.data(), kdf_input.size());
...
{
  public:
    SM2_Decryption_Operation(const SM2_Encryption_PrivateKey &key, RandomNumberGenerator &rng,
                             const std::string &kdf_hash)
        : m_key(key), m_rng(rng), m_kdf_hash(kdf_hash)
    {
        std::unique_ptr<HashFunction> hash = HashFunction::create_or_throw(m_kdf_hash);
        m_hash_size = hash->output_length();
    }
    ...
    secure_vector<uint8_t> decrypt(uint8_t &valid_mask, const uint8_t ciphertext[], size_t ciphertext_len) override
    {
        ...
        const PointGFp dbC1 = group.blinded_var_point_multiply(C1, m_key.private_value(), m_rng, m_ws);
