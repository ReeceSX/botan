...
Ed25519_PublicKey::Ed25519_PublicKey(const AlgorithmIdentifier &, const std::vector<uint8_t> &key_bits)
{
    m_public = key_bits;
...
Ed25519_PrivateKey::Ed25519_PrivateKey(const AlgorithmIdentifier &, const secure_vector<uint8_t> &key_bits)
{
...
{
  public:
    Ed25519_Hashed_Verify_Operation(const Ed25519_PublicKey &key, const std::string &hash, bool rfc8032) : m_key(key)
    {
    ...
    bool is_valid_signature(const uint8_t sig[], size_t sig_len) override
    {
        ...
        const std::vector<uint8_t> &pub_key = m_key.get_public_key();
        BOTAN_ASSERT_EQUAL(pub_key.size(), 32, "Expected size");
        return ed25519_verify(msg_hash.data(), msg_hash.size(), sig, pub_key.data(), m_domain_sep.data(),
                              m_domain_sep.size());
    }
...
{
  public:
    Ed25519_Hashed_Sign_Operation(const Ed25519_PrivateKey &key, const std::string &hash, bool rfc8032) : m_key(key)
    {
        m_hash = HashFunction::create_or_throw(hash);
    ...
    secure_vector<uint8_t> sign(RandomNumberGenerator &) override
    {
        ...
        std::vector<uint8_t> msg_hash(m_hash->output_length());
        m_hash->final(msg_hash.data());
        ed25519_sign(sig.data(), msg_hash.data(), msg_hash.size(), m_key.get_private_key().data(), m_domain_sep.data(),
                     m_domain_sep.size());
        return sig;
    }
...
std::unique_ptr<PK_Ops::Verification> Ed25519_PublicKey::create_verification_op(const std::string &params,
...
std::unique_ptr<PK_Ops::Signature> Ed25519_PrivateKey::create_signature_op(RandomNumberGenerator &,
