...
EC_PublicKey::EC_PublicKey(const EC_Group &dom_par, const PointGFp &pub_point)
...
EC_PublicKey::EC_PublicKey(const AlgorithmIdentifier &alg_id, const std::vector<uint8_t> &key_bits)
...
bool EC_PublicKey::check_key(RandomNumberGenerator &rng, bool) const
{
...
{
    if (enc != PointGFp::COMPRESSED && enc != PointGFp::UNCOMPRESSED && enc != PointGFp::HYBRID)
 ...
 */
EC_PrivateKey::EC_PrivateKey(RandomNumberGenerator &rng, const EC_Group &ec_group, const BigInt &x,
...
EC_PrivateKey::EC_PrivateKey(const AlgorithmIdentifier &alg_id, const secure_vector<uint8_t> &key_bits,
                             bool with_modular_inverse)
{
