...
class BOTAN_PUBLIC_API(2, 0) PK_Decryptor
{
  public:
     ...
     * @return decrypted message
     */
    template <typename Alloc> secure_vector<uint8_t> decrypt(const std::vector<uint8_t, Alloc> &in) const
    {
     ...
     */
    secure_vector<uint8_t> decrypt_or_random(const uint8_t in[], size_t length, size_t expected_pt_len,
                                             RandomNumberGenerator &rng, const uint8_t required_content_bytes[],
                                             const uint8_t required_content_offsets[], size_t required_contents) const;
...
class BOTAN_PUBLIC_API(2, 0) PK_Signer final
{
  public:
     ...
     */
    template <typename Alloc> void update(const std::vector<uint8_t, Alloc> &in)
    {
...
class BOTAN_PUBLIC_API(2, 0) PK_Verifier final
{
  public:
     ...
     * @return true if the signature is valid, false otherwise
     */
    template <typename Alloc> bool check_signature(const std::vector<uint8_t, Alloc> &sig)
    {
...
class BOTAN_PUBLIC_API(2, 0) PK_Key_Agreement final
{
  public:
     ...
     */
    SymmetricKey derive_key(size_t key_len, const uint8_t in[], size_t in_len, const uint8_t params[],
...
class BOTAN_PUBLIC_API(2, 0) PK_KEM_Encryptor final
{
  public:
     ...
     * @param provider the provider to use
     */
    PK_KEM_Encryptor(const Public_Key &key, RandomNumberGenerator &rng, const std::string &kem_param = "",
                     const std::string &provider = "");
...
class BOTAN_PUBLIC_API(2, 0) PK_KEM_Decryptor final
{
  public:
     ...
     * @param provider the provider to use
     */
    PK_KEM_Decryptor(const Private_Key &key, RandomNumberGenerator &rng, const std::string &kem_param = "",
                     const std::string &provider = "");
