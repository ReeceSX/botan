...
XMSS_Verification_Operation::XMSS_Verification_Operation(const XMSS_PublicKey &public_key)
    : m_pub_key(public_key), m_hash(public_key.xmss_hash_function()), m_msg_buf(0)
{
}
                                                                        ...
                                                                        const secure_vector<uint8_t> &seed)
{
    ...
    XMSS_WOTS_PublicKey pub_key_ots(m_pub_key.wots_parameters().oid(), msg, sig.tree().ots_signature(), adrs, seed);
    ...
    for (size_t k = 0; k < params.tree_height(); k++)
    {
        ...
        if (((next_index / (static_cast<size_t>(1) << k)) & 0x01) == 0)
        {
            ...
            XMSS_Common_Ops::randomize_tree_hash(node[1], node[0], sig.tree().authentication_path()[k], adrs, seed,
                                                 m_hash, params);
        }
        else
        {
            adrs.set_tree_index((adrs.get_tree_index() - 1) >> 1);
            XMSS_Common_Ops::randomize_tree_hash(node[1], sig.tree().authentication_path()[k], node[0], adrs, seed,
                                                 m_hash, params);
        }
        node[0] = node[1];
    }
