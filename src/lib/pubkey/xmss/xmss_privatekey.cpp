...
XMSS_PrivateKey::XMSS_PrivateKey(const secure_vector<uint8_t> &key_bits)
    : XMSS_PublicKey(unlock(key_bits)), m_wots_priv_key(m_wots_params.oid(), m_public_seed),
      m_hash(xmss_hash_function()), m_index_reg(XMSS_Index_Registry::get_instance())
{
    /*
...
XMSS_PrivateKey::XMSS_PrivateKey(XMSS_Parameters::xmss_algorithm_t xmss_algo_id, RandomNumberGenerator &rng)
    : XMSS_PublicKey(xmss_algo_id, rng), m_wots_priv_key(XMSS_PublicKey::m_xmss_params.ots_oid(), public_seed(), rng),
      m_hash(xmss_hash_function()), m_prf(rng.random_vec(XMSS_PublicKey::m_xmss_params.element_size())),
...
XMSS_PrivateKey::XMSS_PrivateKey(XMSS_Parameters::xmss_algorithm_t xmss_algo_id, size_t idx_leaf,
                                 const secure_vector<uint8_t> &wots_priv_seed, const secure_vector<uint8_t> &prf,
                                 const secure_vector<uint8_t> &root, const secure_vector<uint8_t> &public_seed)
    : XMSS_PublicKey(xmss_algo_id, root, public_seed),
      ...
      m_hash(XMSS_PublicKey::m_xmss_params.hash_function_name()), m_prf(prf),
      m_index_reg(XMSS_Index_Registry::get_instance())
{
...
secure_vector<uint8_t> XMSS_PrivateKey::tree_hash(size_t start_idx, size_t target_node_height, XMSS_Address &adrs)
{
    ...
    BOTAN_ASSERT((last_idx - start_idx) % subtrees == 0, "Number of worker threads in tree_hash need to divide range "
                                                         "of calculated nodes.");
    ...
    for (size_t i = 0; i < subtrees; i++)
    {
        ...
        work.push_back(thread_pool.run(work_fn, this, std::ref(nodes[i]), start_idx + i * offs,
                                       target_node_height - split_level, std::ref(node_addresses[i]),
                                       std::ref(xmss_hash[i])));
    }
    ...
    while (level-- > 1)
    {
        ...
        for (size_t i = 0; i < (static_cast<size_t>(1) << level); i++)
        {
            ...
            work.push_back(thread_pool.run(&XMSS_Common_Ops::randomize_tree_hash, std::ref(nodes[i]),
                                           std::cref(ro_nodes[2 * i]), std::cref(ro_nodes[2 * i + 1]),
                                           std::ref(node_addresses[i]), std::cref(this->public_seed()),
                                           std::ref(xmss_hash[i]), std::cref(m_xmss_params)));
        }
    ...
    XMSS_Common_Ops::randomize_tree_hash(nodes[0], nodes[0], nodes[1], node_addresses[0], this->public_seed(), m_hash,
                                         m_xmss_params);
    return nodes[0];
#else
    secure_vector<uint8_t> result;
...
void XMSS_PrivateKey::tree_hash_subtree(secure_vector<uint8_t> &result, size_t start_idx, size_t target_node_height,
                                        XMSS_Address &adrs, XMSS_Hash &hash)
{
    ...
    for (size_t i = start_idx; i < last_idx; i++)
    {
        ...
        while (level > 0 && node_levels[level] == node_levels[level - 1])
        {
            ...
            XMSS_Common_Ops::randomize_tree_hash(nodes[level - 1], nodes[level - 1], nodes[level], adrs, seed, hash,
                                                 m_xmss_params);
            node_levels[level - 1]++;
...
std::shared_ptr<Atomic<size_t>> XMSS_PrivateKey::recover_global_leaf_index() const
{
    BOTAN_ASSERT(m_wots_priv_key.private_seed().size() == XMSS_PublicKey::m_xmss_params.element_size() &&
...
secure_vector<uint8_t> XMSS_PrivateKey::raw_private_key() const
{
    ...
    std::copy(m_wots_priv_key.private_seed().begin(), m_wots_priv_key.private_seed().end(), std::back_inserter(result));

    return result;
}

std::unique_ptr<PK_Ops::Signature> XMSS_PrivateKey::create_signature_op(RandomNumberGenerator &, const std::string &,
                                                                        const std::string &provider) const
{
    if (provider == "base" || provider.empty())
        return std::unique_ptr<PK_Ops::Signature>(new XMSS_Signature_Operation(*this));
