...
class BOTAN_PUBLIC_API(2, 0) XMSS_PublicKey : public virtual Public_Key
{
  public:
    ...
    XMSS_PublicKey(XMSS_Parameters::xmss_algorithm_t xmss_oid, const secure_vector<uint8_t> &root,
        ...
        : m_xmss_params(xmss_oid), m_wots_params(m_xmss_params.ots_oid()), m_root(root), m_public_seed(public_seed)
    {
    }
    ...
    std::unique_ptr<PK_Ops::Verification> create_verification_op(const std::string &,
 ...
 *     https://datatracker.ietf.org/doc/rfc8391/
 **/
class BOTAN_PUBLIC_API(2, 0) XMSS_PrivateKey final : public virtual XMSS_PublicKey, public virtual Private_Key
{
  public:
    /**
     ...
     **/
    XMSS_PrivateKey(XMSS_Parameters::xmss_algorithm_t xmss_algo_id, size_t idx_leaf,
    ...
    std::unique_ptr<PK_Ops::Signature> create_signature_op(RandomNumberGenerator &, const std::string &,
                                                           const std::string &provider) const override;
    ...
    {
        return XMSS_PublicKey::size() + sizeof(uint32_t) + 2 * XMSS_PublicKey::m_xmss_params.element_size();
    }
