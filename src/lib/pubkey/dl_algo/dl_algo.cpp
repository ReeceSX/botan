...
{
    return AlgorithmIdentifier(get_oid(), m_group.DER_encode(group_format()));
}
...
DL_Scheme_PublicKey::DL_Scheme_PublicKey(const DL_Group &group, const BigInt &y) : m_y(y), m_group(group)
{
}

DL_Scheme_PublicKey::DL_Scheme_PublicKey(const AlgorithmIdentifier &alg_id, const std::vector<uint8_t> &key_bits,
...
DL_Scheme_PrivateKey::DL_Scheme_PrivateKey(const AlgorithmIdentifier &alg_id, const secure_vector<uint8_t> &key_bits,
                                           DL_Group::Format format)
{
 ...
 */
bool DL_Scheme_PublicKey::check_key(RandomNumberGenerator &rng, bool strong) const
{
 ...
 */
bool DL_Scheme_PrivateKey::check_key(RandomNumberGenerator &rng, bool strong) const
{
