...
polyn_gf2m::polyn_gf2m(polyn_gf2m const &other) : m_deg(other.m_deg), coeff(other.coeff), m_sp_field(other.m_sp_field)
{
}

polyn_gf2m::polyn_gf2m(int d, std::shared_ptr<GF2m_Field> sp_field) : m_deg(-1), coeff(d + 1), m_sp_field(sp_field)
{
}
...
polyn_gf2m::polyn_gf2m(std::shared_ptr<GF2m_Field> sp_field) : m_deg(-1), coeff(1), m_sp_field(sp_field)
{
}
...
polyn_gf2m::polyn_gf2m(size_t t, RandomNumberGenerator &rng, std::shared_ptr<GF2m_Field> sp_field)
    : m_deg(static_cast<int>(t)), coeff(t + 1), m_sp_field(sp_field)
{
    this->set_coef(t, 1);
    for (;;)
    {
...
std::vector<polyn_gf2m> syndrome_init(polyn_gf2m const &generator, std::vector<gf2m> const &support, int n)
{
    ...
    for (j = 0; j < n; j++)
    {
        ...
        {
            (*&result[j])
                .set_coef(i, (generator)[i + 1] ^ m_sp_field->gf_mul(lex_to_gray(support[j]), result[j][i + 1]));
        }
