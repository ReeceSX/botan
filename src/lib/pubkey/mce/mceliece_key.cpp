...
McEliece_PrivateKey::McEliece_PrivateKey(polyn_gf2m const &goppa_polyn,
    ...
    : McEliece_PublicKey(public_matrix, goppa_polyn.get_degree(), inverse_support.size()), m_g{goppa_polyn},
...
{
  public:
    MCE_KEM_Encryptor(const McEliece_PublicKey &key, const std::string &kdf) : KEM_Encryption_with_KDF(kdf), m_key(key)
    {
    }
...
{
  public:
    MCE_KEM_Decryptor(const McEliece_PrivateKey &key, const std::string &kdf) : KEM_Decryption_with_KDF(kdf), m_key(key)
    {
    }
...
std::unique_ptr<PK_Ops::KEM_Encryption> McEliece_PublicKey::create_kem_encryption_op(RandomNumberGenerator & /*rng*/,
