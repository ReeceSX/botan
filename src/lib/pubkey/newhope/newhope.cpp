    ...
    for (size_t i = 0; i < 10; i += 2)
    {
        ...
        for (size_t start = 0; start < distance; start++)
        {
            ...
            for (size_t j = start; j < PARAM_N - 1; j += 2 * distance)
            {
                ...
                a[j + distance] =
                    montgomery_reduce((W * (static_cast<uint32_t>(temp) + 3 * PARAM_Q - a[j + distance])));
            }
        }
...
inline void encode_b(uint8_t *r, const poly *b, const poly *c)
{
    ...
    for (size_t i = 0; i < PARAM_N / 4; i++)
    {
        r[NEWHOPE_POLY_BYTES + i] = static_cast<uint8_t>(c->coeffs[4 * i] | (c->coeffs[4 * i + 1] << 2) |
...
inline void rec(uint8_t *key, const poly *v, const poly *c)
{
    ...
    for (size_t i = 0; i < 256; i++)
    {
        ...
        const int32_t tmp3 =
            16 * PARAM_Q + 8 * static_cast<int32_t>(v->coeffs[768 + i]) - PARAM_Q * (c->coeffs[768 + i]);
