...
class CurveGFp_Montgomery final : public CurveGFp_Repr
{
  public:
    ...
    const BigInt &get_1_rep() const override
    {
        return m_r;
    }

    bool is_one(const BigInt &x) const override
    {
        return x == m_r;
    }
    ...
    void curve_mul_words(BigInt &z, const word x_words[], const size_t x_size, const BigInt &y,
...
void CurveGFp_Montgomery::curve_mul_words(BigInt &z, const word x_w[], size_t x_size, const BigInt &y,
                                          secure_vector<word> &ws) const
{
    ...
    bigint_mul(z.mutable_data(), z.size(), x_w, x_size, std::min(m_p_words, x_size), y.data(), y.size(),
...
void CurveGFp_NIST::curve_mul_words(BigInt &z, const word x_w[], size_t x_size, const BigInt &y,
                                    secure_vector<word> &ws) const
{
    BOTAN_DEBUG_ASSERT(y.sig_words() <= m_p_words);
...
class CurveGFp_P192 final : public CurveGFp_NIST
{
  ...
  private:
    void redc_mod_p(BigInt &x, secure_vector<word> &ws) const override
    {
        redc_p192(x, ws);
    }
};
