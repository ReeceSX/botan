...
class ECIES_ECDH_KA_Operation final : public PK_Ops::Key_Agreement_with_KDF
{
  public:
    ...
    secure_vector<uint8_t> raw_agree(const uint8_t w[], size_t w_len) override
    {
        ...
        const PointGFp S = group.blinded_var_point_multiply(input_point, m_key.private_value(), m_rng, m_ws);
...
PK_Key_Agreement create_key_agreement(const PK_Key_Agreement_Key &private_key, const ECIES_KA_Params &ecies_params,
                                      bool for_encryption, RandomNumberGenerator &rng)
{
...
ECIES_System_Params::ECIES_System_Params(const EC_Group &domain, const std::string &kdf_spec,
                                         ...
                                         PointGFp::Compression_Type compression_type, ECIES_Flags flags)
    : ECIES_KA_Params(domain, kdf_spec, dem_key_len + mac_key_len, compression_type, flags), m_dem_spec(dem_algo_spec),
      m_dem_keylen(dem_key_len), m_mac_spec(mac_spec), m_mac_keylen(mac_key_len)
{
...
ECIES_Encryptor::ECIES_Encryptor(const PK_Key_Agreement_Key &private_key, const ECIES_System_Params &ecies_params,
    ...
    : m_ka(private_key, ecies_params, true, rng), m_params(ecies_params),
