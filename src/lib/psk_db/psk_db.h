...
class BOTAN_PUBLIC_API(2, 4) PSK_Database
{
  public:
    ...
    template <typename Alloc> void set_vec(const std::string &name, const std::vector<uint8_t, Alloc> &psk)
...
{
  public:
    Encrypted_PSK_Database_SQL(const secure_vector<uint8_t> &master_key, std::shared_ptr<SQL_Database> db,
