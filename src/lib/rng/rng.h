...
class BOTAN_PUBLIC_API(2, 0) RandomNumberGenerator
{
  public:
     ...
     * of bits collected.
     */
    virtual size_t reseed(Entropy_Sources &srcs, size_t poll_bits = BOTAN_RNG_RESEED_POLL_BITS,
    ...
    template <typename Alloc> void random_vec(std::vector<uint8_t, Alloc> &v, size_t bytes)
    {
        v.resize(bytes);
...
class BOTAN_PUBLIC_API(2, 0) Serialized_RNG final : public RandomNumberGenerator
{
  public:
    ...
    size_t reseed(Entropy_Sources &src, size_t poll_bits = BOTAN_RNG_RESEED_POLL_BITS,
