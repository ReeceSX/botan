...
const uint32_t P_INIT[18] = {0x243F6A88, 0x85A308D3, 0x13198A2E, 0x03707344, 0xA4093822, 0x299F31D0,
...
{
    return ((S[get_byte(0, X)] + S[256 + get_byte(1, X)]) ^ S[512 + get_byte(2, X)]) + S[768 + get_byte(3, X)];
}
 ...
 */
void Blowfish::salted_set_key(const uint8_t key[], size_t length, const uint8_t salt[], size_t salt_length,
 ...
 */
void Blowfish::generate_sbox(secure_vector<uint32_t> &box, uint32_t &L, uint32_t &R, const uint8_t salt[],
                             size_t salt_length, size_t salt_off) const
{
