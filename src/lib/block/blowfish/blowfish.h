...
class BOTAN_TEST_API Blowfish final : public Block_Cipher_Fixed_Params<8, 1, 56>
{
  public:
     ...
     */
    void salted_set_key(const uint8_t key[], size_t key_length, const uint8_t salt[], size_t salt_length,
  ...
  private:
    ...
    void generate_sbox(secure_vector<uint32_t> &box, uint32_t &L, uint32_t &R, const uint8_t salt[], size_t salt_length,
