...
size_t X942_PRF::kdf(uint8_t key[], size_t key_len, const uint8_t secret[], size_t secret_len, const uint8_t salt[],
    ...
    while (offset != key_len && counter)
    {
        ...
        hash->update(
            DER_Encoder()
                ...
                .encode_if(salt_len != 0, DER_Encoder().start_explicit(0).encode(in, OCTET_STRING).end_explicit())
