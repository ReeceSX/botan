...
extern "C"
{
                        ...
                        size_t time_step)
    {
        ...
        *totp = nullptr;
        ...
        return ffi_guard_thunk(__func__, [=]() -> int {
            *totp = new botan_totp_struct(new Botan::TOTP(key, key_len, hash_algo, digits, time_step));
    ...
    int botan_totp_generate(botan_totp_t totp, uint32_t *totp_code, uint64_t timestamp)
    {
        ...
        return BOTAN_FFI_DO(Botan::TOTP, totp, t, { *totp_code = t.generate_totp(timestamp); });
