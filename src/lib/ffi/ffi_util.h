...
{
  public:
    FFI_Error(const std::string &what, int err_code) : Exception("FFI error", what), m_err_code(err_code)
    {
    }
...
template <typename T, uint32_t MAGIC> struct botan_struct
{
  public:
...
#define BOTAN_FFI_DECLARE_STRUCT(NAME, TYPE, MAGIC)                                                                    \
    ...
    {                                                                                                                  \
        explicit NAME(TYPE *x) : botan_struct(x)                                                                       \
        {                                                                                                              \
        }                                                                                                              \
    }
...
template <typename T, uint32_t M> T &safe_get(botan_struct<T, M> *p)
{
    if (!p)
...
template <typename T, uint32_t M, typename F> int apply_fn(botan_struct<T, M> *o, const char *func_name, F func)
{
    if (!o)
...
#define BOTAN_FFI_DO(T, obj, param, block)                                                                             \
    apply_fn(obj, __func__, [=](T &param) -> int {                                                                     \
        do                                                                                                             \
        {                                                                                                              \
            block                                                                                                      \
        } while (0);                                                                                                   \
...
template <typename T, uint32_t M> int ffi_delete_object(botan_struct<T, M> *obj, const char *func_name)
{
    try
    {
        if (obj == nullptr)
...
template <typename Alloc> int write_vec_output(uint8_t out[], size_t *out_len, const std::vector<uint8_t, Alloc> &buf)
{
