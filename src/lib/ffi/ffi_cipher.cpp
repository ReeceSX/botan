...
extern "C"
{
    ...
    int botan_cipher_get_keyspec(botan_cipher_t cipher, size_t *out_minimum_keylength, size_t *out_maximum_keylength,
    ...
    int botan_cipher_update(botan_cipher_t cipher_obj, uint32_t flags, uint8_t output_ptr[], size_t orig_output_size,
                            size_t *output_written, const uint8_t input_ptr[], size_t orig_input_size,
    ...
    {
        return BOTAN_FFI_RETURNING(Botan::Cipher_Mode, cipher, c, { return c.valid_nonce_length(nl) ? 1 : 0; });
    }
    ...
    {
        return BOTAN_FFI_DO(Botan::Cipher_Mode, cipher, c, { return write_str_output(name, name_len, c.name()); });
    }
}
