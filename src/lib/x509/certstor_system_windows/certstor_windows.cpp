 ...
 */
template <class T> class Handle_Guard
{
  public:
  ...
  private:
    template <class T2 = T> typename std::enable_if<std::is_same<T2, PCCERT_CONTEXT>::value>::type close()
    {
        if (m_context)
        {
    ...
    template <class T2 = T> typename std::enable_if<std::is_same<T2, HCERTSTORE>::value>::type close()
    {
        if (m_context)
        {
...
HCERTSTORE open_cert_store(const char *cert_store_name)
{
    ...
    if (!store)
    {
        throw Botan::Internal_Error("failed to open windows certificate store '" + std::string(cert_store_name) +
                               ...
                               bool return_on_first_found)
{
    ...
    for (const auto store_name : cert_store_names)
    {
        ...
        while (cert_context.assign(
            CertFindCertificateInStore(windows_cert_store.get(), PKCS_7_ASN_ENCODING | X509_ASN_ENCODING,
                                       WINCRYPT_UNUSED_PARAM, find_type, &blob, cert_context.get())))
        {
...
bool already_contains_certificate(const Cert_Vector &certs, Cert_Pointer cert)
{
    return std::any_of(certs.begin(), certs.end(),
                       [&](std::shared_ptr<const Botan::X509_Certificate> c) { return *c == *cert; });
}

Cert_Vector find_cert_by_dn_and_key_id(const Botan::X509_DN &subject_dn, const std::vector<uint8_t> &key_id,
                                       bool return_on_first_found)
{
...
std::vector<X509_DN> Certificate_Store_Windows::all_subjects() const
{
    ...
    for (const auto store_name : cert_store_names)
    {
        ...
        // Handle_Guard::assign exchanges the underlying pointer. No RAII is needed here, because the Windows API takes
        // care of freeing the previous context.
...
Cert_Vector Certificate_Store_Windows::find_all_certs(const X509_DN &subject_dn,
                                                      const std::vector<uint8_t> &key_id) const
{
