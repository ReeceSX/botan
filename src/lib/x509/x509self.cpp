...
void load_info(const X509_Cert_Options &opts, X509_DN &subject_dn, AlternativeName &subject_alt)
{
    ...
    subject_alt.add_othername(OID::from_string("PKIX.XMPPAddr"), opts.xmpp, UTF8_STRING);
 ...
 */
X509_Certificate create_self_signed_cert(const X509_Cert_Options &opts, const Private_Key &key,
                                         const std::string &hash_fn, RandomNumberGenerator &rng)
{
    ...
    extensions.add_new(new Cert_Extension::Basic_Constraints(opts.is_CA, opts.path_limit), true);
    ...
    return X509_CA::make_cert(signer.get(), rng, sig_algo, pub_key, opts.start, opts.end, subject_dn, subject_dn,
                              extensions);
}
 ...
 */
PKCS10_Request create_cert_req(const X509_Cert_Options &opts, const Private_Key &key, const std::string &hash_fn,
