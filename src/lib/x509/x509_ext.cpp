 ...
 */
std::unique_ptr<Certificate_Extension> Extensions::create_extn_obj(const OID &oid, bool critical,
...
void Certificate_Extension::validate(const X509_Certificate &, const X509_Certificate &,
                                     ...
                                     std::vector<std::set<Certificate_Status_Code>> &, size_t)
{
}
...
std::vector<std::pair<std::unique_ptr<Certificate_Extension>, bool>> Extensions::extensions() const
{
    ...
    {
        exts.push_back(
            std::make_pair(std::unique_ptr<Certificate_Extension>(ext.second.obj().copy()), ext.second.is_critical()));
    }
    return exts;
}
...
{
    std::vector<uint8_t> output;
    DER_Encoder(output).start_cons(SEQUENCE).encode(m_key_id, OCTET_STRING, ASN1_Tag(0), CONTEXT_SPECIFIC).end_cons();
    return output;
}
...
{
    BER_Decoder(in).start_cons(SEQUENCE).decode_optional_string(m_key_id, OCTET_STRING, 0);
}
...
{
    std::vector<uint8_t> output;
    DER_Encoder(output).start_cons(SEQUENCE).encode_list(m_oids).end_cons();
    return output;
}
...
void Name_Constraints::validate(const X509_Certificate &subject, const X509_Certificate &issuer,
                                ...
                                std::vector<std::set<Certificate_Status_Code>> &cert_status, size_t pos)
{
...
class Policy_Information final : public ASN1_Object
{
  public:
    ...
    {
        codec.start_cons(SEQUENCE).encode(m_oid).end_cons();
    }

    void decode_from(BER_Decoder &codec) override
    {
        codec.start_cons(SEQUENCE).decode(m_oid).discard_remaining().end_cons();
    }
...
std::vector<uint8_t> Certificate_Policies::encode_inner() const
{
    ...
    std::vector<uint8_t> output;
    DER_Encoder(output).start_cons(SEQUENCE).encode_list(policies).end_cons();
    return output;
}
...
void CRL_Distribution_Points::Distribution_Point::decode_from(class BER_Decoder &ber)
{
    ber.start_cons(SEQUENCE)
        ...
        .decode_optional_implicit(m_point, ASN1_Tag(0), ASN1_Tag(CONTEXT_SPECIFIC | CONSTRUCTED), SEQUENCE, CONSTRUCTED)
        .end_cons()
        .end_cons();
}
