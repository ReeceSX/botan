 ...
 */
void X509_DN::add_attribute(const std::string &type, const std::string &str)
{
...
void X509_DN::encode_into(DER_Encoder &der) const
{
    ...
    else
    {
        ...
        {
            der.start_cons(SET).start_cons(SEQUENCE).encode(dn.first).encode(dn.second).end_cons().end_cons();
        }
    }
...
void X509_DN::decode_from(BER_Decoder &source)
{
    ...
    source.start_cons(SEQUENCE).raw_bytes(bits).end_cons();
    ...
    while (sequence.more_items())
    {
        ...
        while (rdn.more_items())
        {
            ...
            rdn.start_cons(SEQUENCE)
                ...
                .end_cons()
                .verify_end("Invalid X509_DN, data follows RDN");
