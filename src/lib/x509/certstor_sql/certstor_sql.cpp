...
Certificate_Store_In_SQL::Certificate_Store_In_SQL(std::shared_ptr<SQL_Database> db, const std::string &passwd,
                                                   RandomNumberGenerator &rng, const std::string &table_prefix)
    : m_rng(rng), m_database(db), m_prefix(table_prefix), m_password(passwd)
{
    m_database->create_table("CREATE TABLE IF NOT EXISTS " + m_prefix + "certificates (                \
...
std::shared_ptr<const X509_Certificate> Certificate_Store_In_SQL::find_cert(const X509_DN &subject_dn,
...
std::vector<std::shared_ptr<const X509_Certificate>> Certificate_Store_In_SQL::find_all_certs(
    const X509_DN &subject_dn, const std::vector<uint8_t> &key_id) const
{
    ...
    {
        auto blob = stmt->get_blob(0);
        certs.push_back(std::make_shared<X509_Certificate>(std::vector<uint8_t>(blob.first, blob.first + blob.second)));
    }

    return certs;
}

std::shared_ptr<const X509_Certificate> Certificate_Store_In_SQL::find_cert_by_pubkey_sha1(
    const std::vector<uint8_t> & /*key_hash*/) const
{
    throw Not_Implemented("Certificate_Store_In_SQL::find_cert_by_pubkey_sha1");
}

std::shared_ptr<const X509_Certificate> Certificate_Store_In_SQL::find_cert_by_raw_subject_dn_sha256(
...
std::shared_ptr<const X509_CRL> Certificate_Store_In_SQL::find_crl_for(const X509_Certificate &subject) const
{
...
bool Certificate_Store_In_SQL::insert_cert(const X509_Certificate &cert)
{
    ...
    auto stmt = m_database->new_statement("INSERT OR REPLACE INTO " + m_prefix + "certificates (\
                                         fingerprint,          \
                                         subject_dn,           \
                                         key_id,               \
                                         priv_fingerprint,     \
...
std::vector<std::shared_ptr<const X509_Certificate>> Certificate_Store_In_SQL::find_certs_for_key(
    const Private_Key &key) const
{
