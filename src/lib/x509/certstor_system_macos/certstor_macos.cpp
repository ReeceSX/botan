 ...
 */
template <typename T> class scoped_CFType
{
  public:
...
template <typename T> void check_notnull(const T &value, const std::string context)
{
    if (value)
    {
        return;
...
class Certificate_Store_MacOS_Impl
{
    ...
    class Query
    {
      ...
      public:
        ...
        scoped_CFType<CFDictionaryRef> prepare(const CFArrayRef &keychains, const SecPolicyRef &policy)
        {
            ...
            auto query = scoped_CFType<CFDictionaryRef>(
                CFDictionaryCreate(kCFAllocatorDefault, (const void **)m_keys.data(), (const void **)m_values.data(),
                                   m_keys.size(), &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks));
  ...
  public:
    Certificate_Store_MacOS_Impl()
        : m_policy(SecPolicyCreateBasicX509()), m_system_roots(nullptr), m_system_chain(nullptr), m_keychains(nullptr)
    {
...
std::vector<X509_DN> Certificate_Store_MacOS::all_subjects() const
{
    ...
    std::transform(certificates.cbegin(), certificates.cend(), std::back_inserter(output),
                   [](const std::shared_ptr<const X509_Certificate> cert) { return cert->subject_dn(); });

    return output;
}

std::shared_ptr<const X509_Certificate> Certificate_Store_MacOS::find_cert(const X509_DN &subject_dn,
...
std::shared_ptr<const X509_Certificate> Certificate_Store_MacOS::find_cert_by_pubkey_sha1(
...
std::shared_ptr<const X509_Certificate> Certificate_Store_MacOS::find_cert_by_raw_subject_dn_sha256(
