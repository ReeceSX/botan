...
{
    codec.start_cons(SEQUENCE).encode(m_oid).start_cons(SET).raw_bytes(m_parameters).end_cons().end_cons();
}
...
{
    codec.start_cons(SEQUENCE).decode(m_oid).start_cons(SET).raw_bytes(m_parameters).end_cons().end_cons();
}
