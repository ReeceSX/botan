...
class BOTAN_PUBLIC_API(2, 0) Certificate_Store
{
  public:
     ...
     */
    virtual std::shared_ptr<const X509_Certificate> find_cert(const X509_DN &subject_dn,
     ...
     * @return a matching certificate or nullptr otherwise
     */
    virtual std::shared_ptr<const X509_Certificate> find_cert_by_pubkey_sha1(
        const std::vector<uint8_t> &key_hash) const = 0;
     ...
     * @return a matching certificate or nullptr otherwise
     */
    virtual std::shared_ptr<const X509_Certificate> find_cert_by_raw_subject_dn_sha256(
...
class BOTAN_PUBLIC_API(2, 0) Certificate_Store_In_Memory final : public Certificate_Store
{
  public:
     ...
     */
    std::shared_ptr<const X509_Certificate> find_cert(const X509_DN &subject_dn,
    ...
    std::shared_ptr<const X509_Certificate> find_cert_by_pubkey_sha1(
        const std::vector<uint8_t> &key_hash) const override;

    std::shared_ptr<const X509_Certificate> find_cert_by_raw_subject_dn_sha256(
