...
X509_CRL::X509_CRL(const X509_DN &issuer, const X509_Time &this_update, const X509_Time &next_update,
...
std::unique_ptr<CRL_Data> decode_crl_body(const std::vector<uint8_t> &body, const AlgorithmIdentifier &sig_algo)
{
    ...
    if (version != 0 && version != 1)
        throw X509_CRL::X509_CRL_Error("Unknown X.509 CRL version " + std::to_string(version + 1));
