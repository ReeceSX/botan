...
{
  public:
    Asio_SocketUDP(const std::string &hostname, const std::string &service, std::chrono::microseconds timeout)
...
{
  public:
    BSD_SocketUDP(const std::string &hostname, const std::string &service, std::chrono::microseconds timeout)
        : m_timeout(timeout)
    {
        socket_init();
        ...
        if (m_socket == invalid_socket())
        {
            throw System_Error("Connecting to " + hostname + " for service " + service + " failed", errno);
        }
    }
    ...
    void write(const uint8_t buf[], size_t len) override
    {
        ...
        while (sent_so_far != len)
        {
            ...
            socket_op_ret_type sent =
                ::sendto(m_socket, cast_uint8_ptr_to_char(buf + sent_so_far), static_cast<sendrecv_len_type>(left), 0,
...
std::unique_ptr<OS::SocketUDP> OS::open_socket_udp(const std::string &hostname, const std::string &service,
...
std::unique_ptr<OS::SocketUDP> OS::open_socket_udp(const std::string &uri_string, std::chrono::microseconds timeout)
{
