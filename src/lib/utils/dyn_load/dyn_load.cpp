...
void raise_runtime_loader_exception(const std::string &lib_name, const char *msg)
{
    const std::string ex_msg = "Failed to load " + lib_name + ": " + (msg ? msg : "Unknown error");
...
Dynamically_Loaded_Library::Dynamically_Loaded_Library(const std::string &library) : m_lib_name(library), m_lib(nullptr)
{
#if defined(BOTAN_TARGET_OS_HAS_POSIX1)
...
void *Dynamically_Loaded_Library::resolve_symbol(const std::string &symbol)
{
    ...
    if (!addr)
        throw Invalid_Argument("Failed to resolve symbol " + symbol + " in " + m_lib_name);
