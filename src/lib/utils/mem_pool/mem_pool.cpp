...
template <typename T> size_t find_set_bit(T b)
{
...
class BitMap final
{
  ...
  private:
    ...
    enum
    {
        BITMASK_BITS = BOTAN_MP_WORD_BITS
    };
#endif
...
class Bucket final
{
  public:
    Bucket(uint8_t *mem, size_t mem_size, size_t item_size)
        : m_item_size(item_size), m_page_size(mem_size), m_range(mem), m_bitmap(mem_size / item_size), m_is_full(false)
    {
    }
