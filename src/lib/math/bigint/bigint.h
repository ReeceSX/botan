...
class BOTAN_PUBLIC_API(2, 0) BigInt final
{
  public:
     ...
     */
    template <typename Alloc> explicit BigInt(const std::vector<uint8_t, Alloc> &vec) : BigInt(vec.data(), vec.size())
    {
    }
     ...
     */
    template <typename Alloc> void binary_decode(const std::vector<uint8_t, Alloc> &buf)
    {
     ...
     * @result BigInt representing the integer in the byte array
     */
    template <typename Alloc> static BigInt decode(const std::vector<uint8_t, Alloc> &buf)
    {
        return BigInt(buf);
    }
     ...
     * @result BigInt representing the integer in the byte array
     */
    template <typename Alloc> static BigInt decode(const std::vector<uint8_t, Alloc> &buf, Base base)
    {
        if (base == Binary)
            return BigInt(buf);
...
template <> inline void swap<Botan::BigInt>(Botan::BigInt &x, Botan::BigInt &y)
{
    x.swap(y);
}
