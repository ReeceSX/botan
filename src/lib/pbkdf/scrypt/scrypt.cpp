...
std::unique_ptr<PasswordHash> Scrypt_Family::tune(size_t output_length, std::chrono::milliseconds msec,
...
void Scrypt::derive_key(uint8_t output[], size_t output_len, const char *password, size_t password_len,
                        const uint8_t salt[], size_t salt_len) const
{
    scrypt(output, output_len, password, password_len, salt, salt_len, N(), r(), p());
}
...
void scrypt(uint8_t output[], size_t output_len, const char *password, size_t password_len, const uint8_t salt[],
            size_t salt_len, size_t N, size_t r, size_t p)
{
    ...
    pbkdf2(*hmac_sha256.get(), B.data(), B.size(), salt, salt_len, 1);
    ...
    pbkdf2(*hmac_sha256.get(), output, output_len, B.data(), B.size(), 1);
}
