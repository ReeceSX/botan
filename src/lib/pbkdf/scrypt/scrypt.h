 ...
 */
void BOTAN_PUBLIC_API(2, 8) scrypt(uint8_t output[], size_t output_len, const char *password, size_t password_len,
                                   const uint8_t salt[], size_t salt_len, size_t N, size_t r, size_t p);
 ...
 * Scrypt uses approximately (p + N + 1) * 128 * r bytes of memory
 */
inline void scrypt(uint8_t output[], size_t output_len, const std::string &password, const uint8_t salt[],
                   size_t salt_len, size_t N, size_t r, size_t p)
{
    return scrypt(output, output_len, password.c_str(), password.size(), salt, salt_len, N, r, p);
}
