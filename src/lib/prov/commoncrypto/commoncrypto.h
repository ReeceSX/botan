...
{
  public:
    CommonCrypto_Error(const std::string &what) : Exception(what + " failed."), m_rc(0)
    {
    }

    CommonCrypto_Error(const std::string &what, int32_t status)
        : Exception(what + std::string(" failed. Status: ") + ccryptorstatus_to_string(status)), m_rc(status)
    {
    }
