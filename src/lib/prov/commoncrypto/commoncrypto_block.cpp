...
CommonCrypto_BlockCipher::CommonCrypto_BlockCipher(const std::string &algo_name, const CommonCryptor_Opts &opts)
...
void CommonCrypto_BlockCipher::key_schedule(const uint8_t key[], size_t length)
{
    ...
    status = CCCryptorCreate(kCCEncrypt, m_opts.algo, kCCOptionECBMode, full_key.data(), full_key.size(), nullptr,
                             &m_encrypt);
    if (status != kCCSuccess)
    {
        throw CommonCrypto_Error("CCCryptorCreate encrypt", status);
    }
    status = CCCryptorCreate(kCCDecrypt, m_opts.algo, kCCOptionECBMode, full_key.data(), full_key.size(), nullptr,
                             &m_decrypt);
    if (status != kCCSuccess)
    {
