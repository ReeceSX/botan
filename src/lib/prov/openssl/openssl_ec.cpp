...
class OpenSSL_ECDSA_Verification_Operation final : public PK_Ops::Verification_with_EMSA
{
  public:
    ...
    bool verify(const uint8_t msg[], size_t msg_len, const uint8_t sig_bytes[], size_t sig_len) override
    {
...
class OpenSSL_ECDSA_Signing_Operation final : public PK_Ops::Signature_with_EMSA
{
  public:
    OpenSSL_ECDSA_Signing_Operation(const ECDSA_PrivateKey &ecdsa, const std::string &emsa)
        : PK_Ops::Signature_with_EMSA(emsa), m_ossl_ec(nullptr, ::EC_KEY_free)
    {
...
class OpenSSL_ECDH_KA_Operation final : public PK_Ops::Key_Agreement_with_KDF
{
  public:
    ...
    secure_vector<uint8_t> raw_agree(const uint8_t w[], size_t w_len) override
    {
        ...
        std::unique_ptr<EC_POINT, std::function<void(EC_POINT *)>> pub_key(::EC_POINT_new(group), ::EC_POINT_free);

        if (!pub_key)
            throw OpenSSL_Error("EC_POINT_new", ERR_get_error());

        const int os2ecp_rc = ::EC_POINT_oct2point(group, pub_key.get(), w, w_len, nullptr);

        if (os2ecp_rc != 1)
            throw OpenSSL_Error("EC_POINT_oct2point", ERR_get_error());

        const int ecdh_rc = ::ECDH_compute_key(out.data(), out.size(), pub_key.get(), m_ossl_ec.get(),
                                               /*KDF*/ nullptr);
