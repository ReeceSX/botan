...
{
  public:
    OpenSSL_Cipher_Mode(const std::string &name, const EVP_CIPHER *cipher, Cipher_Dir direction);
    ~OpenSSL_Cipher_Mode();
...
OpenSSL_Cipher_Mode::OpenSSL_Cipher_Mode(const std::string &name, const EVP_CIPHER *algo, Cipher_Dir direction)
    : m_mode_name(name), m_direction(direction), m_key_set(false), m_nonce_set(false)
{
    ...
    if (!EVP_CipherInit_ex(m_cipher, algo, nullptr, nullptr, nullptr, m_direction == ENCRYPTION ? 1 : 0))
...
void OpenSSL_Cipher_Mode::finish(secure_vector<uint8_t> &buffer, size_t offset)
{
    verify_key_set(m_key_set);
    BOTAN_STATE_CHECK(m_nonce_set);
