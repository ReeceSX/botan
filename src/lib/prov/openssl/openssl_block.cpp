...
class OpenSSL_BlockCipher final : public BlockCipher
{
  public:
    ...
    OpenSSL_BlockCipher(const std::string &name, const EVP_CIPHER *cipher, size_t kl_min, size_t kl_max, size_t kl_mod);
...
OpenSSL_BlockCipher::OpenSSL_BlockCipher(const std::string &algo_name, const EVP_CIPHER *algo)
    : m_block_sz(EVP_CIPHER_block_size(algo)), m_cipher_key_spec(EVP_CIPHER_key_length(algo)), m_cipher_name(algo_name),
      m_key_set(false)
{
...
OpenSSL_BlockCipher::OpenSSL_BlockCipher(const std::string &algo_name, const EVP_CIPHER *algo, size_t key_min,
                                         size_t key_max, size_t key_mod)
    : m_block_sz(EVP_CIPHER_block_size(algo)), m_cipher_key_spec(key_min, key_max, key_mod), m_cipher_name(algo_name),
      m_key_set(false)
{
    if (EVP_CIPHER_mode(algo) != EVP_CIPH_ECB_MODE)
        throw Invalid_Argument("OpenSSL_BlockCipher: Non-ECB EVP was passed in");
...
void OpenSSL_BlockCipher::key_schedule(const uint8_t key[], size_t length)
{
    ...
    else
    {
        ...
        if (EVP_CIPHER_CTX_set_key_length(m_encrypt, ilength) == 0 ||
            ...
            throw Invalid_Argument("OpenSSL_BlockCipher: Bad key length for " + m_cipher_name);
    }
...
{
    return new OpenSSL_BlockCipher(m_cipher_name, EVP_CIPHER_CTX_cipher(m_encrypt),
                                   m_cipher_key_spec.minimum_keylength(), m_cipher_key_spec.maximum_keylength(),
