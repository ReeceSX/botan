...
class OpenSSL_RSA_Encryption_Operation final : public PK_Ops::Encryption
{
  public:
    ...
    secure_vector<uint8_t> encrypt(const uint8_t msg[], size_t msg_len, RandomNumberGenerator &) override
    {
        ...
        int rc = ::RSA_public_encrypt(static_cast<int>(inbuf.size()), inbuf.data(), outbuf.data(), m_openssl_rsa.get(),
                                      m_padding);
        if (rc < 0)
...
class OpenSSL_RSA_Verification_Operation final : public PK_Ops::Verification_with_EMSA
{
  public:
    OpenSSL_RSA_Verification_Operation(const RSA_PublicKey &rsa, const std::string &emsa)
        : PK_Ops::Verification_with_EMSA(emsa), m_openssl_rsa(nullptr, ::RSA_free)
    {
        const std::vector<uint8_t> der = rsa.public_key_bits();
        const uint8_t *der_ptr = der.data();
        m_openssl_rsa.reset(::d2i_RSAPublicKey(nullptr, &der_ptr, der.size()));
    ...
    secure_vector<uint8_t> verify_mr(const uint8_t msg[], size_t msg_len) override
    {
        ...
        int rc = ::RSA_public_decrypt(static_cast<int>(inbuf.size()), inbuf.data(), outbuf.data(), m_openssl_rsa.get(),
                                      RSA_NO_PADDING);
        if (rc < 0)
...
class OpenSSL_RSA_Signing_Operation final : public PK_Ops::Signature_with_EMSA
{
  public:
    OpenSSL_RSA_Signing_Operation(const RSA_PrivateKey &rsa, const std::string &emsa)
        : PK_Ops::Signature_with_EMSA(emsa), m_openssl_rsa(nullptr, ::RSA_free)
    {
        const secure_vector<uint8_t> der = rsa.private_key_bits();
        const uint8_t *der_ptr = der.data();
        m_openssl_rsa.reset(d2i_RSAPrivateKey(nullptr, &der_ptr, der.size()));
    ...
    secure_vector<uint8_t> raw_sign(const uint8_t msg[], size_t msg_len, RandomNumberGenerator &) override
    {
        const size_t mod_sz = ::RSA_size(m_openssl_rsa.get());
        ...
        int rc = ::RSA_private_encrypt(static_cast<int>(inbuf.size()), inbuf.data(), outbuf.data(), m_openssl_rsa.get(),
                                       RSA_NO_PADDING);
        if (rc < 0)
...
std::unique_ptr<PK_Ops::Encryption> make_openssl_rsa_enc_op(const RSA_PublicKey &key, const std::string &params)
{
...
std::unique_ptr<PK_Ops::Decryption> make_openssl_rsa_dec_op(const RSA_PrivateKey &key, const std::string &params)
{
    auto pad_info = get_openssl_enc_pad(params);
    return std::unique_ptr<PK_Ops::Decryption>(new OpenSSL_RSA_Decryption_Operation(key, pad_info.first));
}

std::unique_ptr<PK_Ops::Verification> make_openssl_rsa_ver_op(const RSA_PublicKey &key, const std::string &params)
{
    return std::unique_ptr<PK_Ops::Verification>(new OpenSSL_RSA_Verification_Operation(key, params));
}

std::unique_ptr<PK_Ops::Signature> make_openssl_rsa_sig_op(const RSA_PrivateKey &key, const std::string &params)
{
    return std::unique_ptr<PK_Ops::Signature>(new OpenSSL_RSA_Signing_Operation(key, params));
}

std::unique_ptr<RSA_PrivateKey> make_openssl_rsa_private_key(RandomNumberGenerator &rng, size_t rsa_bits)
{
    if (rsa_bits > INT_MAX)
    ...
    std::free(der);
    return std::unique_ptr<Botan::RSA_PrivateKey>(new RSA_PrivateKey(AlgorithmIdentifier(), keydata));
}
} // namespace Botan
