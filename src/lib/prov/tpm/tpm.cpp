...
void tss_error(TSS_RESULT res, const char *expr, const char *file, int line)
{
    ...
    err << "TPM error " << Trspi_Error_String(res) << " layer " << Trspi_Error_Layer(res) << " in " << expr << " at "
        << file << ":" << line;
...
void set_policy_secret(TSS_HPOLICY policy, const char *secret)
{
    if (secret)
    {
        ...
        TSPI_CHECK_SUCCESS(::Tspi_Policy_SetSecret(policy, TSS_SECRET_MODE_PLAIN, std::strlen(secret), as_b));
    }
    else
    {
...
TPM_PrivateKey::TPM_PrivateKey(TPM_Context &ctx, size_t bits, const char *key_password) : m_ctx(ctx)
{
    ...
    TSPI_CHECK_SUCCESS(::Tspi_SetAttribUint32(key, TSS_TSPATTRIB_KEY_INFO, TSS_TSPATTRIB_KEYINFO_SIGSCHEME,
...
TPM_PrivateKey::TPM_PrivateKey(TPM_Context &ctx, const std::string &uuid_str, TPM_Storage_Type storage_type)
...
TPM_PrivateKey::TPM_PrivateKey(TPM_Context &ctx, const std::vector<uint8_t> &blob) : m_ctx(ctx)
{
    TSPI_CHECK_SUCCESS(::Tspi_Context_LoadKeyByBlob(m_ctx.handle(), m_ctx.srk(), blob.size(),
                                                    const_cast<uint8_t *>(blob.data()), &m_key));
...
std::vector<uint8_t> TPM_PrivateKey::public_key_bits() const
{
    ...
    DER_Encoder(bits).start_cons(SEQUENCE).encode(get_n()).encode(get_e()).end_cons();
    return bits;
}
...
{
  public:
    TPM_Signing_Operation(const TPM_PrivateKey &key, const std::string &hash_name)
        : m_key(key), m_hash(HashFunction::create(hash_name)), m_hash_id(pkcs_hash_id(hash_name))
    {
    }
