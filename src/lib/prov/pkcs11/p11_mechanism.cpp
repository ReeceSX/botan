...
// maps a PSS mechanism type to the number of bytes used for the salt, the mechanism type of the underlying hash
// algorithm and the MGF
...
{
    explicit MechanismData(MechanismType _type) : type(_type)
    {
    }
...
struct RSA_SignMechanism final : public MechanismData
{
    ...
    // if the signature mechanism includes hashing, then this value must match the hash algorithm indicated by the
    // signature mechanism
    MechanismType hash;
    ...
    // length, in bytes, of the salt value used in the PSS encoding; typical values are the length of the message hash
    // and zero
    size_t salt_size;
};
...
static std::map<std::string, MechanismType> EcdsaHash = {
    {"Raw", MechanismType::Ecdsa},           {"SHA-160", MechanismType::EcdsaSha1},
    {"SHA-224", MechanismType::EcdsaSha224}, {"SHA-256", MechanismType::EcdsaSha256},
    {"SHA-384", MechanismType::EcdsaSha384}, {"SHA-512", MechanismType::EcdsaSha512}};
...
static std::map<std::string, KeyDerivation> EcdhHash = {
    {"Raw", KeyDerivation::Null},          {"SHA-160", KeyDerivation::Sha1Kdf},
    {"SHA-224", KeyDerivation::Sha224Kdf}, {"SHA-256", KeyDerivation::Sha256Kdf},
    {"SHA-384", KeyDerivation::Sha384Kdf}, {"SHA-512", KeyDerivation::Sha512Kdf}};
} // namespace
...
MechanismWrapper MechanismWrapper::create_rsa_crypt_mechanism(const std::string &padding)
{
    ...
    {
        // at this point it would be possible to support additional configurations that are not predefined above by
        // parsing `padding`
