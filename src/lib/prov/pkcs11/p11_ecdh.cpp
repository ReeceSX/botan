...
class PKCS11_ECDH_KA_Operation final : public PK_Ops::Key_Agreement
{
  public:
    ...
    /// Applications relying only on a V2.20 encoding (e.g. the DER variant) other than the one specified now (raw) may
    /// not work with all V2.30 compliant tokens.
...
std::unique_ptr<PK_Ops::Key_Agreement> PKCS11_ECDH_PrivateKey::create_key_agreement_op(
    RandomNumberGenerator &, const std::string &params, const std::string & /*provider*/) const
{
