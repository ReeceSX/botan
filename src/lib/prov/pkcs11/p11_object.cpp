...
void AttributeContainer::add_class(ObjectClass object_class)
{
    ...
    add_attribute(AttributeType::Class, reinterpret_cast<uint8_t *>(&m_numerics.back()),
...
void AttributeContainer::add_string(AttributeType attribute, const std::string &value)
{
    ...
    add_attribute(attribute, reinterpret_cast<const uint8_t *>(m_strings.back().data()),
...
void AttributeContainer::add_attribute(AttributeType attribute, const uint8_t *value, uint32_t size)
{
    ...
    for (auto &existing_attribute : m_attributes)
    {
        ...
        {
            // remove old entries
            m_strings.erase(std::remove_if(m_strings.begin(), m_strings.end(),
                                           [&existing_attribute](const std::string &data) {
                                               return data.data() == existing_attribute.pValue;
                                           }),
                            m_strings.end());

            m_numerics.erase(std::remove_if(m_numerics.begin(), m_numerics.end(),
                                            [&existing_attribute](const uint64_t &data) {
                                                return &data == existing_attribute.pValue;
                                            }),
                             m_numerics.end());

            m_vectors.erase(std::remove_if(m_vectors.begin(), m_vectors.end(),
                                           [&existing_attribute](const secure_vector<uint8_t> &data) {
                                               return data.data() == existing_attribute.pValue;
                                           }),
                            m_vectors.end());
    ...
    if (!exists)
    {
        m_attributes.push_back(
            Attribute{static_cast<CK_ATTRIBUTE_TYPE>(attribute), const_cast<uint8_t *>(value), size});
    }
}
...
{
    m_session.get().module()->C_CreateObject(m_session.get().handle(), obj_props.data(),
                                             static_cast<Ulong>(obj_props.count()), &m_handle);
}
...
ObjectHandle Object::copy(const AttributeContainer &modified_attributes) const
{
    ...
    module()->C_CopyObject(m_session.get().handle(), m_handle, modified_attributes.data(),
                           static_cast<Ulong>(modified_attributes.count()), &copied_handle);
