...
bool pss_verify(HashFunction &hash, const secure_vector<uint8_t> &pss_repr, const secure_vector<uint8_t> &message_hash,
                size_t key_bits, size_t *out_salt_size)
{
    const size_t HASH_SIZE = hash.output_length();
...
PSSR::PSSR(HashFunction *h) : m_hash(h), m_salt_size(m_hash->output_length()), m_required_salt_len(false)
{
}

PSSR::PSSR(HashFunction *h, size_t salt_size) : m_hash(h), m_salt_size(salt_size), m_required_salt_len(true)
{
}
...
secure_vector<uint8_t> PSSR::encoding_of(const secure_vector<uint8_t> &msg, size_t output_bits,
 ...
 */
bool PSSR::verify(const secure_vector<uint8_t> &coded, const secure_vector<uint8_t> &raw, size_t key_bits)
{
    size_t salt_size = 0;
...
AlgorithmIdentifier PSSR::config_for_x509(const Private_Key &key, const std::string &cert_hash_name) const
{
...
PSSR_Raw::PSSR_Raw(HashFunction *h) : m_hash(h), m_salt_size(m_hash->output_length()), m_required_salt_len(false)
{
}

PSSR_Raw::PSSR_Raw(HashFunction *h, size_t salt_size) : m_hash(h), m_salt_size(salt_size), m_required_salt_len(true)
{
}
...
secure_vector<uint8_t> PSSR_Raw::encoding_of(const secure_vector<uint8_t> &msg, size_t output_bits,
                                             RandomNumberGenerator &rng)
{
 ...
 */
bool PSSR_Raw::verify(const secure_vector<uint8_t> &coded, const secure_vector<uint8_t> &raw, size_t key_bits)
{
    size_t salt_size = 0;
    const bool ok = pss_verify(*m_hash, coded, raw, key_bits, &salt_size);
